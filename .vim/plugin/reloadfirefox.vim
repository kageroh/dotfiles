...
function! SetMozreplReloadFirefox()
    if exists('g:reload_firefox')
        ...
        augroup mozreplreloadfirefox
           autocmd BufWritePost *.html,*.htm,*.php,*.js,*.css,*.scss,*.xml,*.xsl call ReloadFirefox()
        augroup END
        let g:reload_firefox=1
        echo "Firefox browser content is reloaded when file(*.html,*htm,*.php,*.js,*.css,*.scss,*.xml,*.xsl) is saved."
    endif
endfunction
